...
public class AdjustedHistoricalDataManager {
	...
	public static TreeMap<Date,Quote> getAdjustedHistoricalQuotes(Contract contract,BarSize barSize){
		...
		if(originMap.isEmpty() || !adjustedMap.isEmpty()){
	...
	private static TreeMap<Date,Quote> calculateAndStoreAdjustedHistoricalQuotes(Contract contract, BarSize barSize){
		...
		TreeMap<Date,Quote> originMap = originalHistoricalData.getQuotes(barSize);
		TreeMap<Date,Double> adjustedCloseMap = originalHistoricalData.getAdjustedClose();
		Iterator<Entry<Date,Quote>> originIterator = originMap.entrySet().iterator();
		...
		Entry<Date,Quote> currentQuoteEntry = originIterator.next();
		Entry<Date,Double> currentAdjustedCloseEntry = adjustedCloseIterator.next();
		while(true){
			Date quoteDate = currentQuoteEntry.getKey();
			Date adjustedCloseDate = currentAdjustedCloseEntry.getKey();
			if(quoteDate.equals(adjustedCloseDate)){
				Quote currentQuote = currentQuoteEntry.getValue();
				Double ratio = currentAdjustedCloseEntry.getValue()/currentQuote.getClose();
				adjustedMap.put(quoteDate, new Quote(quoteDate, currentQuote.getOpen()*ratio, currentQuote.getClose()*ratio, currentQuote.getHigh()*ratio, currentQuote.getLow()*ratio, currentQuote.getVolume()/ratio));
				if(!originIterator.hasNext() || !adjustedCloseIterator.hasNext()) break;
				currentQuoteEntry = originIterator.next();
				currentAdjustedCloseEntry = adjustedCloseIterator.next();
			}else if(quoteDate.before(adjustedCloseDate)){
				if(!originIterator.hasNext()) break;
				currentQuoteEntry  = originIterator.next();
			}else{
